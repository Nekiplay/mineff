    ...
    async run(bot, args, message) {
        ...
        bot.on("physicTick", () => {
            ...
            for (const entity of Object.values(bot.entities)) {
                const dist = bot.entity.position.distanceSquared(
                    entity.position
                );
        ...
        app.use(express.static("./components/web/public"));

        server.listen(config.attack.webPort, () => {
            console.log(
                `Web server listening on port ${config.attack.webPort}`
            );
        });
        ...
        function attack(entity) {
            if (
                ...
                if (start - new Date().getTime() <= 0 && !food) {
                    start =
                        new Date().getTime() +
                        getRandomInt(delay_min, delay_max);
